...
import type { CreateTask, Task } from '@/lib/interfaces/taskInterfaces';

export default async function CreateTaskService(data: CreateTask): Promise<Task> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        const msg = errorData?.message || 'Dados inválidos para criação da tarefa';
        if (typeof window !== "undefined") {
          const { toast } = await import("sonner");
          toast.error(msg);
        }
      ...
      if (response.status === 404) {
        const msg = 'Trilha de conhecimento ou curso não encontrado';
        if (typeof window !== "undefined") {
          const { toast } = await import("sonner");
          toast.error(msg);
        }

...
import type { UpdateKnowledgeTrail } from "@/lib/interfaces/knowledgeTrailInterfaces";

export default async function UpdateKnowledgeTrailService(
    knowledgeTrailId: number,
    trailForm: UpdateKnowledgeTrail
) {
    ...
    try {
        ...
        const response = await fetch(`${baseURL}/knowledge-trail/${knowledgeTrailId}`, {
            method: "PUT",
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                const msg = "Trilha de conhecimento não encontrada.";

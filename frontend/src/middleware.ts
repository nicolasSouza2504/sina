...
function authGuard(req: NextRequest): { ok: boolean; user: AuthPayload | null; response?: NextResponse } {
    ...
    if (!user) {
        ...
        res.cookies.set(TOKEN_COOKIE, "", {path: "/", maxAge: 0, httpOnly: true, secure: true})
...
function roleGuard(req: NextRequest, user: AuthPayload | null): NextResponse | null {
    const {pathname} = req.nextUrl
